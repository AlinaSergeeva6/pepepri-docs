...
1. **Create a Global Variable:**\
   Use `this.variable` in a Transaction Header Calculated Field to store the UDT data. For example, `this.priceBookUDTList` will hold the `PriceBook` UDT data.
2. **Fetch UDT Data Once:**\
   Retrieve the UDT data once and store it in the global variable. Use a unique identifier (e.g., `WrntyID`) to ensure the data is refreshed only when necessary.
