...
async function onVisitLoadScript(data: any) {
	...
	if (data?.data && data.data.AccountUUID && data.data.Visits?.length) {
		...
		const hasOpenVisitInAnotherAccount = await hasOpenVisitOnAnotherAccount(
			accountUUID
		);
		...
		if (!hasOpenVisitInAnotherAccount) {
			...
			try {
				...
				userInfo = await pepperi.resources
					.resource("UserInfo")
					.key(userUUID)
					.get();
			} catch (error: any) {
	...
	async function hasOpenVisitOnAnotherAccount(accountUUID: string) {
				...
				RightNode: {
					...
					RightNode: {
						...
						RightNode: {
							...
							FieldType: "String",
							Operation: "Contains",
							Values: [VISIT_FLOW_MAIN_ACTIVITY],
						},
					},
				},
			},
		...
		if (res.objects?.length) {
			...
			if (hasOpenVisit) {
				...
				await client.alert(
					"Visit In Progress",
					"Please finish visit on account " + accountName
				);
			}
		}
	...
	function filterProfileAndSalesChannel(
		visits: Visit[],
		profile: string,
		salesChannel: string
	) {
			...
			for (let group of visit.Groups) {
				for (let i = group.Steps.length - 1; i >= 0; --i) {
					...
					if (group.Steps[i]?.profileSelection?.length) {
						const item = group.Steps[i].profileSelection.find(
							(p) => p === profile
						);
						if (!item) {
					...
					if (group.Steps[i]?.salesChannelSelector?.length) {
						const item = group.Steps[i].salesChannelSelector.find(
			...
			for (let group of visit.Groups) {
				...
				for (let i = 0; i < group.Steps.length; i++) {
					...
					if (step.Resource == "transactions") {
						// get the catalogs
						const res = await pepperi.resources
							.resource("TransactionsCatalogsRelation")
							.search({
						...
						if (res?.Objects?.length) {
							...
							if (transactionCatalogs.catalogsExternalID.length == 1) {
								...
								for (let catalog of transactionCatalogs.catalogsExternalID) {
									...
									const profilesRes = await pepperi.resources
										.resource("catalogs")
										.search({
											Fields: ["TSAProfiles"],
	...
	async function filterByFrequency(visits: any[], accountUUID: string) {
			...
			for (let j = 0; j < visits[i].Groups.length; j++) {
				for (let h = 0; h < visits[i].Groups[j].Steps.length; h++) {
					...
					if (
						...
						const stepActivities: any = await getLastExecutedInstance(
							accountUUID,
							step
						);
		...
		if (promiseList.length) {
			for (let i = 0; i < promiseList.length; i++) {
				visits[promiseList[i].iIndex].Groups[
					promiseList[i].jIndex
				].Steps.splice(promiseList[i].hIndex, 1);
			}
		}
	}
		...
		const filterObj: any = {
			...
			RightNode: {
				...
				LeftNode: {
					...
					FieldType: "String",
					Operation: "Contains",
					Values: [accountUUIDStr],
				},
				RightNode: {
					ApiName: "Type",
					FieldType: "String",
					Operation: "Contains",
					Values: [resourceCreationData],
				},
			},
		};
		...
		switch (resource) {
			case "activities":
				...
				if (res?.success && res.objects?.length) {
					res.objects = res.objects.filter((act) =>
			...
			case "transactions":
				...
				if (res?.success && res.objects?.length) {
					...
					res.objects = res.objects.filter(
						(act) =>
    ...
    */
							...
							case "activities": {
								{
									...
									step.Completed =
										activityObjects.find((obj) =>
											step.CompletedStatusName.some(
												(e) => e === obj?.StatusName
											)
										) != undefined;
									step.Disabled = step.MaxCount
										? activityObjects.length >= step.MaxCount
							...
							case "transactions": {
								{
										...
										step.BaseActivities.map(async (transactionUUID) => {
											...
											if (transactionUUID) {
												...
												transactionObj = {
													...
													...(await getTransaction(transactionUUID, fields))
														.object,
												} as any;
											}
									...
									const stepTransactions = transactionObjects.filter(
										(transactionObj) => {
											let res = false;
											debugger;
											if (transactionObj) {
												const catalogExID = transactionObj.CatalogExternalID;
												const tsaVisitData = !!transactionObj.TSAVisitData
													? transactionObj.TSAVisitData
													: "";
												const stepVisitData = !!step.additionalVisitData
													? step.additionalVisitData
													: "";
												if (
									...
									debugger;
									step.BaseActivities = stepTransactions.map(
										(obj) => obj!.UUID
									);
									step.Completed =
										stepTransactions.find((obj) =>
											step.CompletedStatusName.some(
												(e) => e === obj?.StatusName
											)
										) != undefined;
									step.Disabled = step.MaxCount
										? stepTransactions.length >= step.MaxCount
										: false;
								}
								break;
							}
							default: {
								{
									const surveyObjects = await Promise.all(
										step.BaseActivities.map(async (surveyUUID) => {
											...
											if (surveyUUID) {
												surveyObj = {
													UUID: surveyUUID,
													...(await getSurvey(step.Resource, surveyUUID))
														.Objects[0],
												} as any;
											}
											return surveyObj;
									...
									step.Completed =
										surveyObjects.find((obj) =>
											step.CompletedStatusName.some(
												(e) => e === obj?.StatusName
											)
										) != undefined;
									step.Disabled = step.MaxCount
										? surveyObjects.length >= step.MaxCount
										: false;
								}
								break;
							}
	...
	async function getUserUUID(accountUUID) {
		...
		if (resource.status === "added") {
			...
			userUUID = await (
				await getActivity(resource.id, fields)
			).object.CreatorUUID;
		}
		return userUUID;
	}
